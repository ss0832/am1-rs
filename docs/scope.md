...
| **CPHF iteration limit and tolerance, from both APIs** | ✅ since 0.2.3 — `cphf_max_iter`/`cphf_tol` on `Am1Options` and `PbcOptions`, reaching `hessian`, `frequencies`, `ir_spectrum`, `vibrations` and the ASE calculator. They were private constants (100/`1e-9` molecular, 200/`1e-8` periodic), so a system whose response needed more passes could not be given them without editing the crate. Separate from `max_scf` deliberately: the response is a different fixed point, converging at its own rate and failing for its own reasons |
| **A missing atomic mass is named where it is missing** | ✅ since 0.2.3 — `data_tables::atomic_mass` refuses a zero instead of dividing by it. `MASS[80]` was `0.0`, so every mass-weighted quantity for mercury became `−inf` and the first thing to notice was the eigensolver reporting `NoConvergence` |
| L-BFGS geometry optimization | ✅ molecules; and **under a cell since 0.2.3** (`pbc::optimize_periodic`, `native.pbc_optimize`, CLI `optimize` with a cell) — scaled coordinates plus a strain, on the k-point forces and, with `relax_cell`, the analytic stress at an optional target pressure. Strain components on non-periodic axes are frozen by construction |
...
| **Wavefunction output, Molden format** (`[Atoms]`, `[GTO]`/`[STO]`, `[MO]`) | ✅ **`[GTO]` by default since 0.2.3** — a run-time-fitted Gaussian expansion of the Slater shells, overlapping them by better than 0.99999, because `[STO]` is exact for this basis and read by almost no viewer. The coefficients are transformed by `S^{-1/2}` so they belong to the functions the file declares, instead of sitting in NDDO's *assumed* orthonormal basis; `--molden-basis sto` and `--molden-orthogonal` restore the 0.2.2 file. See [theory.md](theory.md) |
...
| **Phonon eigenvectors** | ✅ since 0.2.3 — `ForceConstants::modes`, `native.phonons(eigenvectors=True)`, `AM1.get_phonons(eigenvectors=True)`. Two conventions, because they answer different questions: `polarization` is the orthonormal `e(q)`, for sums over modes; `displacements` is `e_a/√m_a`, what an atom does and what to add to a geometry, **not** renormalized so the heavy/light amplitude ratio survives. Columns are modes, ordered like `frequencies_cm`; complex because away from Γ the pattern carries a Bloch phase. Opt-in — `n_q · (3N)²` complex numbers. Through 0.2.2 a mode could not be followed from a phonon result at all, which is why this release's Al₂(CH₃)₆ saddle had to borrow its direction from the molecular Hessian |
| **A response on a partially occupied ground state** | ⛔ **refused since 0.2.3**, and this is the honest entry rather than a gap that stays quiet. There is no `∂f/∂ε` term in the coupled-perturbed equations, so a partially filled band is a term they do not have: the CPHF path drops such a band from *both* the occupied and the virtual set (losing its whole orbital-relaxation contribution) and DFPT freezes the `f_n(k) − f_m(k+q)` weighting every band pair. Through 0.2.2 both ran and returned a plausible number. `Am1Error::FractionalOccupation` names the k-point, the band and the occupation; `allow_fractional_occupations` restores the old path and its wrong answer. **Not a gate on smearing** — the judgement is on the converged occupations, so cubic BN at 4×4×4 / `kT = 0.3` is accepted and AlP at 6×6×6 / `kT = 0` is too, both measured in `tests/fractional_occupations.rs` |
| **Adaptive SCF fallback** (Kerker, smearing anneal) | ✅ since 0.2.3, `PbcOptions::adaptive`. The controller reads the residual trace: growing or oscillating → Kerker preconditioning, mixing to 0.1, DIIS history discarded; decaying at a ratio above 0.99 → raise the electronic temperature, **then anneal it back**, since smearing moves the fixed point rather than the path and `converged` must mean converged at the requested smearing. Kerker is `(1 + κγ)⁻¹` against NDDO's monopole kernel — the plane-wave `q²/(q²+q₀²)` in the only form available without a plane-wave grid — applied to the net charge per atom alone, which is the part that sloshes. `PbcResult::fallback` reports `kerker_kappa`, `final_mixing`, `max_smearing_ev` and `diis_resets` |
...
| **Geometry optimization** | ✅ since 0.2.3 — `optimize_divide_conquer`, `native.divide_conquer(optimize=True)`, CLI `optimize --dc`. L-BFGS on the DC gradient, so a structure too large for a full SCF can be relaxed and not only measured. Returns the per-step trace alongside the final geometry |
...
| **Divide-and-conquer from the CLI** | ✅ since 0.2.3 — `--dc` on the `energy`, `gradient` and `optimize` modes, with `--dc-core` and `--dc-buffer` for the subsystem radii. A mode DC does not cover refuses rather than silently running the full calculation, in both front ends |
